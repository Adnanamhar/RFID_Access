...
MFRC522 mfrc522(SDA_PIN, RST_PIN); // setting mfrc dengan dan RST
...
void loop() {
  ...
  Serial.println();
  Serial.print("Mesaage :");
  content.toUpperCase(); // UID dijadikan huruf besar semua

  if (content.substring(1) == "2A EC 32 B4") // masukkan ID kartu / E-KTP disini
  {
    lcd.clear();
